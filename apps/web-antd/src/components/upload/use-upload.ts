 ...
 */
export function useUpload(directory?: string, returnMode: string = 'url') {
  // 后端上传地址
  const uploadUrl = getUploadUrl(returnMode);
  // 是否使用前端直连上传
  ...
  async function httpRequest(
    ...
    if (isClientUpload) {
      ...
      // 模式二：后端上传
      const uploadApi = returnMode === 'id-url' ? uploadFileNew : uploadFile;
      return uploadApi({ file, directory }, onUploadProgress);
    }
  }
 ...
 * 获得上传 URL
 */
export function getUploadUrl(returnMode: string): string {
  if (returnMode === 'id-url') {
    return `${apiURL}/infra/file/upload-new`;
  }
