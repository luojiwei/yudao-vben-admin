...
const { accept, helpText, maxNumber, maxSize, layout } = toRefs(props);
...
async function customRequest(info: UploadRequestOption) {
  ...
  if (!api || !isFunction(api)) {
    api = useUpload(props.directory, props.returnMode).httpRequest;
  }
  try {
    // 上传文件
...
function handleUploadSuccess(res: any, file: File) {
  // 删除临时文件
  const names = new Set(uploadList.value.map((item) => item.name));
  fileList.value = fileList.value?.filter((item) => !names.has(item.name));

  // 添加到临时上传列表
  const fileUrl = res?.url || res?.data || res;
  uploadList.value.push({
    id: props.returnMode === 'id-url' ? res.id : undefined,
    name: file.name,
    url: fileUrl,
    status: UploadResultStatus.DONE,
...
function getValue() {
  ...
  if (props.maxNumber === 1) {
    if (list.length > 0) {
      return isString(props.modelValue) ? list[0] : list;
    } else {
      return props.returnMode === 'id-url' ? {} : '';
    }
  }

  if (props.returnMode !== 'id-url' && isUsingModelValue.value) {
